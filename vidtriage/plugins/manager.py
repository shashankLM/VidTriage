...
1. **Built-ins** — every subpackage of :mod:`vidtriage.plugins.builtin`.
2. **Installed packages** — anything advertising the ``vidtriage.plugins``
   entry-point group, so ``pip install vidtriage-sam3`` is enough to add a model.
3. **Drop-ins** — ``.py`` files and packages under ``~/.vidtriage/plugins/``,
...
PLUGIN_ATTR = "PLUGIN"
ENTRY_POINT_GROUP = "vidtriage.plugins"
...
class PluginManager:
    ...
    def _discover_user_dir(self, directory: Path) -> None:
        ...
        for path in sorted(directory.iterdir()):
            ...
            module_name = f"vidtriage_userplugin_{path.stem}"
            try:
