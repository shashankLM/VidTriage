...
:mod:`legacy import <vidtriage.plugins.builtin.triage.session>` picks up from an
...
class Ledger:
    ...
    means a crash — or a second VidTriage running over the same corpus — cannot
...
def read_log(path: Path) -> list[Decision]:
    ...
    for number, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            if data.get("kind") == "vidtriage-triage-log":
                continue
