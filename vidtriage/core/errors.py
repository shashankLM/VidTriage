...
Everything VidTriage raises deliberately derives from :class:`VidTriageError`, so
...
class VidTriageError(Exception):
...
class MediaError(VidTriageError):
...
class PluginError(VidTriageError):
...
class PersistenceError(VidTriageError):
    """Reading or writing persisted state failed."""


class FileOperationError(VidTriageError):
