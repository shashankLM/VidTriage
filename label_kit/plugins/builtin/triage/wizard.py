...
from ....media.source import IMAGE_EXTENSIONS
...
class SetupWizard(QDialog):
    ...
    def __init__(
        ...
        self.setWindowTitle("label-kit — Setup")
        ...
        self._input_edit = self._add_dir_row(
            layout, "Input directory (videos and images to classify):",
        )
        layout.addSpacing(8)
        self._output_edit = self._add_dir_row(
            layout, "Output directory (where a snapshot is written):",
        )
    ...
    def _update_info(self) -> None:
        ...
        if input_text and Path(input_text).is_dir():
            parts.append(f"Input: {_media_summary(Path(input_text))}")
        ...
        if output_text and Path(output_text).is_dir():
            ...
            if counts or errors:
                ...
                parts.append(f"Output: {total} file(s) ({', '.join(summary)})")
            else:
...
def _media_summary(directory: Path) -> str:
    """``"12 videos, 3 images"`` — broken out so a mixed corpus is obvious."""
    found = discover_media(directory)
    images = sum(1 for p in found if p.suffix.lower() in IMAGE_EXTENSIONS)
    videos = len(found) - images
    parts = [f"{videos} video{'s' * (videos != 1)}"] if videos else []
    if images:
        parts.append(f"{images} image{'s' * (images != 1)}")
