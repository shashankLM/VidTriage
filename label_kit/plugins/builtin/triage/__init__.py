"""Whole-video triage — the original label-kit workflow, as a plugin.

Press a number key, the decision is logged, the next file loads. It contributes
...
rules, and :mod:`snapshot` for turning a log back into folders of media.
...
from .models import MAX_CLASSES, ClassEntry, MediaItem, TriageConfig
...
class TriagePlugin(Plugin):
    id = "triage"
    name = "Triage"
    description = "Classify whole videos and images with the number keys"
    ...
    def _register_static_commands(self, ctx: PluginContext) -> None:
        ...
        ctx.add_command(
            id="triage.error", title="Mark As Error", shortcut="X",
        ...
        ctx.add_command(
            ...
            is_enabled=lambda: self.session is not None,
            description="CSV of every file and the class it was filed under",
        )
        ctx.add_command(
            ...
            description="Copy every classified file into <outdir>/<class>/",
        )
    ...
    def _go_to_next_pending(self) -> None:
        ...
        if not pending:
            ctx.app.status("Everything classified", 6000)
            return
    ...
    def _change_classes(self) -> None:
        ...
        text, accepted = QInputDialog.getMultiLineText(
            ctx.app.window, "label-kit — Classes",
    ...
    def _export_classifications(self) -> None:
        ...
        try:
            with Path(path).open("w", newline="", encoding="utf-8") as handle:
                ...
                writer.writerow(["file", "class", "path"])
    ...
    def _write_snapshot(self) -> None:
        """Copy the log's verdict out as folders of media."""
        ctx = self._ctx
        if ctx is None or self.session is None:
            return
        ...
        link = QMessageBox.question(
            ...
            f"Snapshot {len(plan.pairs)} file(s) into\n{target}\n\n"
