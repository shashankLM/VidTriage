"""Materialise the log as folders of media — the deliverable, built on demand.
...
class SnapshotPlan:
    ...
    pairs: list[tuple[MediaItem, Path]] = field(default_factory=list)
...
def plan_snapshot(items: list[MediaItem], output_dir: Path) -> SnapshotPlan:
    ...
    *,
    ...
    Args:
        link: Prefer hardlinks, falling back to copying per file. See
            :func:`~label_kit.plugins.builtin.triage.io_ops.copy_media`.
    ...
    if not plan.pairs:
        raise FileOperationError(
            "Nothing to snapshot — nothing has been classified yet.",
        )
...
def _collision_message(collisions: dict[str, list[Path]]) -> str:
    lines = [
        "Two or more files would be written to the same place.",
        "",
