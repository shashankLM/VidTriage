...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="label-kit",
        description="Triage videos and images, annotate frames, and run models on regions you point at.",
    ...
    parser.add_argument(
        ...
        help="Skip user drop-in plugins from ~/.labelkit/plugins/",
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Debug logging")
    parser.add_argument("--version", action="version", version=f"label-kit {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    _log.info("label-kit %s starting", __version__)

    # Before anything reads the config directory: the project was called
    # VidTriage until recently, and ~/.vidtriage holds the only record of what
    # previous runs classified.
    migrate_legacy_config()
    ...
    app.setApplicationName("label-kit")
    app.setApplicationVersion(__version__)
    app.setOrganizationName("label-kit")
